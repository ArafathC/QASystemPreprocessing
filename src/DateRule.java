	...
	public String apply(String token) {
		...
		try {
			if (1 == 1) {
				...
				Pattern ddmmyyyyPat = Pattern
						.compile("(\\d{1,2})\\s([^\\s]+?)\\s([\\d]{4})");
				...
				while (ddmmyyyyMatch.find()) {
					...
					if (Month.parse(match) > -1) {
						...
						if (datepart.length() < 2)
							datepart = "0" + datepart;
						ddmmyyyyMatch.appendReplacement(sb,
								ddmmyyyyMatch.group(3) + monthMatcher(match)
										+ datepart);
						ddmmyyyy = true;
					}
				}
				...
				Pattern mmddyyyyPat = Pattern
						.compile("([^\\s]+?)\\s(\\d{1,2}),\\s([\\d]{4})");
				...
				while (mmddyyyyMatch.find()) {
					...
					if (Month.parse(match) > -1) {
						...
						if (datepart.length() < 2)
							datepart = "0" + datepart;
						mmddyyyyMatch.appendReplacement(sb,
								mmddyyyyMatch.group(3) + monthMatcher(match)
										+ datepart);
						mmddyyyy = true;
					}
				}
				...
				Pattern yyyBCPat = Pattern.compile("([\\d]{2,4})\\s(BC|AD)",
				...
				while (yyyBCMatch.find()) {
					...
					if (yyyBCMatch.group(2).trim().toUpperCase()
							.compareTo("BC") == 0)
						match = "-" + match;
				...
				while (timeMatch.find()) {
					...
					timeMatch.appendReplacement(sb, hrs + ":"
							+ timeMatch.group(2).trim() + ":00");
					timeBool = true;
				}
				if (timeBool)
					token = timeMatch.appendTail(sb).toString();

				Pattern yyyyPat = Pattern
						.compile(
								"((\\s|\\.|\\,|\\(|\\)|^)([\\d]{4})(\\s|\\.|\\,|\\(|\\)|$))",
				...
				Pattern totalTimePat = Pattern
						.compile("([\\d]{1,2}:[\\d]{1,2}:[\\d]{1,})\\s?(UTC)?\\s?([^\\s]+?)?\\s?([^\\s]+?)?\\s([\\d]{8})");
				Matcher totalTimeMatch = totalTimePat.matcher(token);
				boolean totalTime = false;
				sb = new StringBuffer();
				while (totalTimeMatch.find()) {
					String match = totalTimeMatch.group(5).trim() + " "
							+ totalTimeMatch.group(1);
				...
				Pattern onlyMonthPat = Pattern
						.compile("([A-Za-z]+?)\\s([\\d]{1,2})(\\s|\\.|\\,|$)");
				...
				while (onlyMonthMatch.find()) {
					...
					if (Month.parse(match) > -1) {
						int dt = Integer.parseInt(onlyMonthMatch.group(2)
								.trim());
						String dttime = new DecimalFormat("00").format(dt);
						onlyMonthMatch.appendReplacement(sb, "1900"
								+ monthMatcher(match) + dttime + " ");
						onlyMonth = true;
					}
				...
				Pattern mulYearsPat = Pattern
						.compile("([\\d]{4})([^\\d\\w\\s\\p{Punct}])(\\d{2})");
				...
				while (mulYearsMatch.find()) {
					...
					String year2 = mulYearsMatch.group(1).trim()
							.substring(0, 2)
							+ mulYearsMatch.group(3).trim();
					mulYearsMatch.appendReplacement(sb, year1 + "0101"
							+ mulYearsMatch.group(2).trim() + year2 + "0101");
					mulYears = true;
				}
				if (mulYears)
