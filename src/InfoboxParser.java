...
public class InfoboxParser {
	static final Pattern releasedatepat = Pattern.compile(
			"\\{\\{([a-zA-Z\\s\\(\\)]+?)\\|([a-zA-Z\\s\\(\\)]+?)\\}\\}",
			Pattern.CASE_INSENSITIVE);
	static final Pattern releasedatepat2 = Pattern
			.compile(
					"\\{\\{([a-zA-Z_\\s]+?)?\\|([0-9]{4})\\|?([0-9]{1,})?\\|?([0-9]{1,})?[\\|\\}]{1,}",
					Pattern.CASE_INSENSITIVE);
	static final Pattern birthdatepat = Pattern
			.compile("\\{\\{([Bb]irth|[Dd]eath)[\\s_]date([^0-9]+?)?\\|([0-9]{4})\\|([0-9]{1,})\\|([0-9]{1,})\\|?([^\\}]+?)?\\}\\}");
	static final Pattern otherdatespat = Pattern
			.compile(
					"\\{\\{([a-zA-Z\\s]+?)[\\s_]?date([^0-9]+?)?\\|([0-9]{4})\\|([0-9]{1,})?\\|?([0-9]{1,})?\\|?([^\\}]+?)?\\}\\}",
					Pattern.CASE_INSENSITIVE);
	static final Pattern linkpat = Pattern.compile(
			"\\[{2}(([^\\|\\]]+?)\\|)?([^\\|\\]]+?)\\]{2}", Pattern.MULTILINE
					+ Pattern.DOTALL);
	static final Pattern speciallinkpat = Pattern.compile(
			"\\[{2}(([^\\,\\(|]+?)((,|\\()[^\\|\\#]+?))\\|\\]{2}",
			Pattern.DOTALL);
	static final Pattern namespacelinkpat = Pattern
			.compile(
					"\\[{2}(([^\\,\\(|]+?):([^\\|\\#]+?))((,|\\()([^\\|\\#]+?))?\\|\\]{2}",
					Pattern.DOTALL);
	static final Pattern externallinkpat = Pattern.compile(
			"\\[htt[p|ps]:([^\\s\\]]+?)(\\s([^\\]]+?))?\\]",
			Pattern.CASE_INSENSITIVE);
	static final Pattern linkmarkuppat = Pattern.compile(
			"\\[\\[([^\\|]+?)\\|?\\]\\]", Pattern.DOTALL + Pattern.MULTILINE);
	static final Pattern filepat = Pattern
			.compile(
					"\\[\\[(\\s)?(Image|ftp|gopher|telnet|file|notes|ms-help|media)(:)(.+?)(\\|([^\\|]+?))?\\]\\]",
					Pattern.CASE_INSENSITIVE);
	static final Pattern textformatpat1 = Pattern
			.compile("\\{\\{([^\\|\\}]+?)\\|([^\\|\\}]+?)[\\|\\}]([^\\}]+?)?\\}{1,}");
	static final Pattern textformatpat2 = Pattern.compile("\\{(.*)\\}",
			Pattern.MULTILINE + Pattern.DOTALL + Pattern.CASE_INSENSITIVE);
	static final Pattern textformatpat3 = Pattern.compile("<!--([^<>]+?)-->",
			...
			for (String s : nodes.keySet()) {
				if (nodes.get(s).trim().compareTo("") != 0)
					...
					s = "<field name="
							...
							+ s
							+ "\" "
							+ "type=\"text_general\" indexed=\"true\" stored=\"true\" multiValued=\"false\"/>";
	...
	public static HashMap<String, String> parseInfoBox(String rawText,
			String title, String id, String infotype) {
		HashMap<String, String> tokens = new HashMap<String, String>();
		Pattern splitpat = Pattern.compile("\\|([A-Za-z_\\s]+?)=(.+?)?$",
		...
		for (String s : linesInfobox) {
			...
			if (!((s.compareTo("") == 0) || s.contains("{{infobox") || s.trim()
					.replace("}}", "").length() < 2)) {
				s = s.replace("\n", " ");
				s = s.replaceAll("<br.>", ",");
				...
				s = parseTemplates(s);
				s = parseLinks(s);
				if (!(s.contains("_date") || s.toLowerCase().contains("date") || s
						.contains("formation"))) {
					s = textformatpat1.matcher(s).replaceAll("$2");
					s = textformatpat2.matcher(s).replaceAll("");
					if (s.contains("}}")) {
						Pattern endpat = Pattern.compile("([^\\}]+?)\\}{2,}$",
								Pattern.MULTILINE + Pattern.DOTALL);
				...
				if (s.matches("\\s?([a-zA-Z0-9_]+?)=([^\\=^\\}\\}]+?)")) {
					...
					data[0] = Pattern
							...
							.replaceAll("$1");
					data[1] = s.replaceAll("\\s?([a-zA-Z0-9_\\s]+?)=(.+?)",
							"$2");
				...
				if (data[0].compareTo("birth_date") == 0
						|| data[0].compareTo("death_date") == 0) {
					String month = "";
					String day = "";
					if (data.length > 1 && data[1].compareTo("") != 0) {
						...
						if (birthdateMatch.matches()) {
							...
							data[1] = birthdateMatch.replaceAll("$3") + month
									+ day;
						}
					}
				}
				if (data[0].contains("_date") || data[0].contains("released")
						|| data[0].contains("formation")) {
					String month = "";
					String day = "";
					if (data.length > 1 && data[1].compareTo("") != 0) {
						data[1] = new DateRule().apply(data[1]);
						Matcher otherdatesMatch = otherdatespat
								.matcher(data[1]);
						if (otherdatesMatch.matches()) {
							if (otherdatesMatch.groupCount() > 4
									&& otherdatesMatch.group(4) != null
									&& otherdatesMatch.group(4).trim()
											.compareTo("") != 0) {
								if (otherdatesMatch.replaceAll("$4").length() <= 1)
									month = "0"
											+ otherdatesMatch.replaceAll("$4");
								else
									month = otherdatesMatch.replaceAll("$4");
							}
							otherdatesMatch = otherdatespat.matcher(data[1]);
							if (otherdatesMatch.groupCount() > 5
									&& otherdatesMatch.replaceAll("$5").trim()
											.compareTo("") != 0) {
								if (otherdatesMatch.replaceAll("$5").length() <= 1)
									day = "0"
											+ otherdatesMatch.replaceAll("$5");
								else
									day = otherdatesMatch.replaceAll("$5");
							}
							data[1] = otherdatesMatch.replaceAll("$3") + month
									+ day;
						}
					}
				}
				...
				if (data.length > 1) {
					if (data[0].compareTo("") != 0) {
						if (data[0].equalsIgnoreCase("years_active")
								|| data[0].equalsIgnoreCase("yearsactive"))
							tokens.put(data[0],
									formatText(yearsFormatText(data[1])));
						else if (data[0].contains("release"))
							tokens.put(
									data[0],
									formatText(parseDates(releaseFormatText(data[1]
											.trim()))));
						else if (data[0].contains("date")
								|| data[0].contains("formation")
								|| data[0].contains("extinction"))
				...
				if (data[0].compareTo("") != 0
						&& !relationsMap.containsKey(data[0])) {
					relationsMap.put(data[0], 1);
				} else {
					if (data[0].compareTo("") != 0)
						relationsMap
								.put(data[0], relationsMap.get(data[0]) + 1);
				}
	...
	public static String releaseFormatText(String s) {
		...
		if (releasedatematch.find()) {
			s = releasedatematch.replaceAll("$2$3$4")
					.replaceAll("[\\|\\s]y[\\|\\s\\}]", "").replace("|", " ")
					.replace("}", "");
		}
		Pattern release = Pattern.compile("([0-9]{4})([0-9]{1,2})([0-9]{1,2})");
		Matcher datematch = release.matcher(s);
		while (datematch.find()) {
			if (datematch.group(2).length() == 1
					&& datematch.group(3).length() == 1) {
				s = s.replace(datematch.group(0), datematch.group(1) + "0"
						+ datematch.group(2) + "0" + datematch.group(3));
			} else if (datematch.group(2).length() > 1
					&& datematch.group(3).length() == 1) {
				if (Integer.parseInt(datematch.group(2)) > 12) {
					s = s.replace(datematch.group(0), datematch.group(1) + "0"
							+ datematch.group(2) + datematch.group(3));
				} else {
					s = s.replace(datematch.group(0), datematch.group(1)
							+ datematch.group(2) + "0" + datematch.group(3));
				}
			}
		}
	...
	public static String formatXMLTag(String s) {
		/*
		 * producer // producers production_company production instrument
		 * //instruments birth_date // born death_date //died death past_members
		 * // former_members writer // writers label //labels preceeded_by
		 * //predecessor alias //also_known_as formation // formed
		 */
		s = s.trim();
		s = s.replaceAll("\\s([a-zA-Z0-9])", "_$1").replace("/", "_")
				.replaceAll("^([0-9](.+?))", "num_$1");
		s = new SpecialCharRule().apply(s);
		if (s.contains("release"))
			...
			s = "caption";
		else if (s.equals("producers") || s.equals("production_company")
				|| s.equals("production"))
			s = "producer";
	...
	public static String parseTagFormatting(String text) {
		...
		Pattern tagPat = Pattern.compile("(\\s<([^<>]+?)>|<([^<>]+?)>)",
				Pattern.MULTILINE + Pattern.DOTALL);
		Pattern tagPat1 = Pattern.compile("(&lt;(.+?)&gt;)", Pattern.MULTILINE
				+ Pattern.DOTALL);
		Pattern tagPat2 = Pattern.compile("&quot", Pattern.MULTILINE
				+ Pattern.DOTALL);
		Pattern tagPat3 = Pattern.compile("&ndash;", Pattern.MULTILINE
				+ Pattern.DOTALL);
		String itemtext = "";
		if (text != null)
			itemtext = text;
